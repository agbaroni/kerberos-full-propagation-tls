...
openssl3 req -passout 'pass:password' -out openldap.csr -new -keyout openldap.key -config /tmp/openssl.cnf -subj "/C=IT/O=Example/CN=mymachine" -nodes
openssl3 ca -config /tmp/openssl.cnf -in openldap.csr -passin 'pass:password' -out openldap.crt -batch

openssl3 req -passout 'pass:password' -out postgresql.csr -new -keyout postgresql.key -config /tmp/openssl.cnf -subj "/C=IT/O=Example/CN=mymachine" -nodes
openssl3 ca -config /tmp/openssl.cnf -in postgresql.csr -passin 'pass:password' -out postgresql.crt -batch

openssl3 req -passout 'pass:password' -out wildfly-backend.csr -new -keyout wildfly-backend.key -config /tmp/openssl.cnf -subj "/C=IT/O=Example/CN=mymachine" -nodes
openssl3 ca -config /tmp/openssl.cnf -in wildfly-backend.csr -passin 'pass:password' -out wildfly-backend.crt -batch

openssl3 req -passout 'pass:password' -out wildfly-frontend.csr -new -keyout wildfly-frontend.key -config /tmp/openssl.cnf -subj "/C=IT/O=Example/CN=mymachine" -nodes
